...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        int end = width - EXITOFFSET;
		for (int x = start; x < end; x++) { //base ground
            this.setBlock(x, DEFAULTHEIGHT - 2, HILL_TOP);
            this.setBlock(x, DEFAULTHEIGHT - 1, GROUND);
        }

        int startingHeight = DEFAULTHEIGHT - 3;
		String chromosome = dna.getChromosome();
		int curr;
		for (int i = 0; i < chromosome.length(); i++) {
		    char c = chromosome.charAt(i);
		    curr = (int)(start + (i * 1.0 / chromosome.length()) * (end - start));
		    if (c == 't') {
		        this.setBlock(curr, startingHeight - 3, COIN);
            } else if (c == 'o') {
                this.setBlock(curr, startingHeight, ROCK);
            } else if (c == 'm' && curr > 5) {
